...
from docker_registry_frontend.manifest import (
...
class DockerRegistry(abc.ABC):
    ...
    def __init__(self, name, url, user=None, password=None):
        ...
        requests_cache.install_cache(
            "requests", expire_after=300, include_get_headers=True
        )
    ...
    def get_number_of_all_layers(self, repo):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            future_to_tag = (
                executor.submit(self.get_number_of_layers_with_tag, repo, tag)
                for tag in tags
            )
    ...
    def get_manifest(self, repo, tag):
        v1manifest = make_manifest(
            self.json_request(
                DockerV2Registry.GET_MANIFEST_TEMPLATE.format(
                    url=self._url, repo=repo, tag=tag
                )
            )
        )
...
class DockerV2Registry(DockerRegistry):
    ...
    def delete_tag(self, repo, tag):
        ...
        try:
            self.request(
                "DELETE",
                DockerV2Registry.GET_MANIFEST_TEMPLATE.format(
                    url=self._url, repo=repo, tag=digest
                ),
            )
    ...
    def is_online(self):
        try:
            resp = self.request("GET", DockerV2Registry.API_BASE.format(url=self._url))
    ...
    def get_repos(self, url_template: str = GET_ALL_REPOS_TEMPLATE):
        ...
        response = self.request("GET", url_template.format(url=self._url))

        repos = json.loads(response.content.decode())["repositories"]
        if "Link" in response.headers:
            next_repos_link = re.search(
                DockerV2Registry.LINK_HEADER_PATTERN, response.headers["Link"]
            )[0]
    ...
    def get_tags(self, repo):
        tags = self.json_request(
            DockerV2Registry.GET_ALL_TAGS_TEMPLATE.format(url=self._url, repo=repo)
        )["tags"]
